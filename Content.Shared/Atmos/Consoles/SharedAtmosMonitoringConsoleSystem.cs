...
public abstract class SharedAtmosMonitoringConsoleSystem : EntitySystem
{
    ...
    {
        Dictionary<Vector2i, Dictionary<(int, AtmosPipeLayer, string), ulong>> chunks;
    ...
    protected sealed class AtmosMonitoringConsoleState(
        ...
        : ComponentState
    {
        public Dictionary<Vector2i, Dictionary<(int, AtmosPipeLayer, string), ulong>> Chunks = chunks;
    ...
    protected sealed class AtmosMonitoringConsoleDeltaState(
        Dictionary<Vector2i, Dictionary<(int, AtmosPipeLayer, string), ulong>> modifiedChunks,
        Dictionary<NetEntity, AtmosDeviceNavMapData> atmosDevices,
        HashSet<Vector2i> allChunks)
        : ComponentState, IComponentDeltaState<AtmosMonitoringConsoleState>
    {
        public Dictionary<Vector2i, Dictionary<(int, AtmosPipeLayer, string), ulong>> ModifiedChunks = modifiedChunks;
        public Dictionary<NetEntity, AtmosDeviceNavMapData> AtmosDevices = atmosDevices;
        ...
        public void ApplyToFullState(AtmosMonitoringConsoleState state)
        {
            ...
            {
                state.Chunks[index] = new Dictionary<(int, AtmosPipeLayer, string), ulong>(data);
            }
        ...
        {
            var chunks = new Dictionary<Vector2i, Dictionary<(int, AtmosPipeLayer, string), ulong>>(state.Chunks.Count);
            ...
            foreach (var (index, data) in state.Chunks)
            {
                ...
                if (ModifiedChunks.ContainsKey(index))
                    chunks[index] = new Dictionary<(int, AtmosPipeLayer, string), ulong>(ModifiedChunks[index]);

                else
                    chunks[index] = new Dictionary<(int, AtmosPipeLayer, string), ulong>(state.Chunks[index]);
            }
