...
public abstract class SharedAtmosMonitoringConsoleSystem : EntitySystem
{
    ...
    {
        Dictionary<Vector2i, Dictionary<(int, byte, string), ulong>> chunks;
    ...
    protected sealed class AtmosMonitoringConsoleState(
        ...
        : ComponentState
    {
        public Dictionary<Vector2i, Dictionary<(int, byte, string), ulong>> Chunks = chunks;
    ...
    {
        public Dictionary<Vector2i, Dictionary<(int, byte, string), ulong>> ModifiedChunks = modifiedChunks;
        public Dictionary<NetEntity, AtmosDeviceNavMapData> AtmosDevices = atmosDevices;
        ...
        public void ApplyToFullState(AtmosMonitoringConsoleState state)
        {
            ...
            {
                state.Chunks[index] = new Dictionary<(int, byte, string), ulong>(data);
            }
        ...
        {
            var chunks = new Dictionary<Vector2i, Dictionary<(int, byte, string), ulong>>(state.Chunks.Count);
            ...
            foreach (var (index, data) in state.Chunks)
            {
                ...
                if (ModifiedChunks.ContainsKey(index))
                    chunks[index] = new Dictionary<(int, byte, string), ulong>(ModifiedChunks[index]);

                else
                    chunks[index] = new Dictionary<(int, byte, string), ulong>(state.Chunks[index]);
            }
